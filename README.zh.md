...
<p align="center">
  <a href="https://nukemai.openbmb.cn"><img src="https://img.shields.io/badge/官网-nukemai.openbmb.cn-FF6B35?style=flat-square&logo=googlechrome&logoColor=white" alt="官方网站"/></a>
  <a href="https://nukemai.openbmb.cn/nukemai.github.io/demo/p/nukemai-demo"><img src="https://img.shields.io/badge/Demo-在线体验-brightgreen?style=flat-square" alt="在线体验"/></a>
...
<p align="center">
  ...
  <br/>
  <a href="https://nukemai.openbmb.cn">官网</a> · <a href="https://nukemai.openbmb.cn/nukemai.github.io/demo/p/nukemai-demo">在线体验</a> · <a href="https://nukemai.openbmb.cn/nukemai.github.io/docs/en/introduction">教程</a> · <a href="#-安装与快速开始">快速开始</a> · <a href="#-核心亮点">核心亮点</a> · <a href="#使用场景">使用场景</a> · <a href="#-联系我们">社区</a>
</p>
...
- **[2026.05.28]** NukemAI 正式开源！欢迎访问官网 [nukemai.openbmb.cn](https://nukemai.openbmb.cn)，期待社区的贡献、反馈与 Star 支持。

---

## 💡 关于 NukemAI

**NukemAI** 是一个以「WorkSpace（工作舱）」为核心设计的开源智能体操作系统，由清华大学 [THUNLP](https://nlp.csai.tsinghua.edu.cn/) 实验室、[面壁智能](https://modelbest.cn/)、[OpenBMB](https://www.openbmb.cn/) 与 [AI9Stars](https://github.com/AI9Stars) 联合研发并开源，面向通用场景、适用于多任务，是 Agent 时代一个真正的「生产力工具」。
...
NukemAI 正是围绕这些问题做的增量探索。它以 WorkSpace 为基本单位，将文件、记忆、技能在项目级别完整隔离与沉淀，并配套提供 **白盒记忆**、**智能路由**、**Always-on** 三大能力，整套系统原生支持 [Model Context Protocol (MCP)](https://modelcontextprotocol.io/)，跨前端（Web / CLI / IM）行为一致。
...
NukemAI 的三大核心能力在实际生产环境中展现出了显著的优势：
...
在传统的黑盒 Agent 中，多任务混居会导致记忆全局污染。NukemAI 通过 WorkSpace 实现了记忆的白盒化管理：
...
<tr>
  ...
  <th width="45%" align="left">NukemAI（白盒）</th>
</tr>
</thead>
<tbody>
<tr>
...
NukemAI 提供了开箱即用的 Web UI，支持完整的 WorkSpace 管理、白盒记忆编辑、以及多智能体协作过程的可视化。

### 使用场景

> 以下所有演示均由端侧模型通过 NukemAI 智能路由完成生成——无需调用云端大模型。
...
如果希望优先使用官方 Node.js 下载地址，也可以通过 `NUKEMAI_NODE_DIST_FALLBACK_MIRRORS` 显式设置一个或多个可信的备用镜像。

```bash
nukemai            # 在 http://localhost:3001 启动服务
nukemai status     # 查看运行状态
```

之后如果想在 macOS / Linux 上再次打开 NukemAI，请在终端运行 `nukemai`，然后在浏览器中打开终端打印的地址。如果当前 shell 还没有刷新 PATH，请新开一个终端，或先 source 对应的 shell 配置文件。
...
PowerShell 安装脚本会使用 `%USERPROFILE%\.nukemai` 下的 Windows 原生路径，检查 Node.js 22.13+ 与 `node:sqlite`，在可用时通过 `winget` 安装缺失依赖，构建 NukemAI，并在 `%USERPROFILE%\.nukemai\bin` 生成 `nukemai.cmd` 启动器。Git LFS 媒体资源对核心功能是可选的；如果 Git LFS 不可用或下载超时，安装脚本会跳过演示视频/GIF 并继续安装。

安装完成后，脚本会启动 NukemAI 并打印 UI 地址，通常是 `http://localhost:3001`。脚本不会自动打开浏览器，请把该地址复制到浏览器中完成初始化配置（Provider + API key）。也可以在 PowerShell 中打开：
...
之后如果想再次打开 NukemAI，请在新的 PowerShell 窗口运行 `nukemai`，然后在浏览器中打开终端打印的地址。如果当前窗口还识别不到 `nukemai`，可以直接运行启动器：
...
NukemAI 使用仓库提交的 `pnpm-lock.yaml` 保证源码安装可复现。请优先使用上面的 `corepack pnpm ...`，不要改用 `npm install`；在 macOS 上，这也能减少原生依赖不必要地回退到源码编译的概率。

**2. 配置模型 Provider**
NukemAI 依赖 `~/.nukemai/nukemai.yaml` 进行配置。您可以手动创建、运行启动脚本自动生成，**或者在启动 Web UI 后直接在设置界面中进行可视化配置**。
...
该命令会初始化 `~/.nukemai/nukemai.yaml`，让 Gateway 可以启动并进入首次 onboarding。随后打开 Web UI，在 onboarding/设置面板中完成 Provider 和 API Key 配置。
...
NukemAI 采用开放的插件架构，插件代码与开源核心严格隔离。开发者可以通过 `plugin.json` 轻松扩展系统能力：
...
感谢所有为 NukemAI 提交代码与反馈的开发者！我们欢迎新的成员加入，共同构建下一代智能体操作系统。
...
- 关于技术问题及功能请求，请提交 [GitHub Issues](https://github.com/OpenBMB/NukemAI/issues)。
- 欢迎加入我们的社区与我们交流：
...
<p align="center">
  NukemAI 由清华大学 <a href="https://nlp.csai.tsinghua.edu.cn/">THUNLP</a>、<a href="https://modelbest.cn/">面壁智能</a>、<a href="https://www.openbmb.cn/">OpenBMB</a> 与 <a href="https://github.com/AI9Stars">AI9Stars</a> 联合研发。
</p>
...
如果您觉得 NukemAI 对您的工作或研究有帮助，请点亮一颗 Star 支持我们！
