...
NukemAI 在容器内由两个协作的 Node.js 进程组成：

- **Gateway**：智能体运行时，监听 `NUKEMAI_GATEWAY_PORT`（默认 `18789`）
...
启动 NukemAI 前，请确认 Docker daemon 正在运行。macOS/Windows 用户需要先启动 Docker Desktop，并等待 engine ready：
...
容器会在镜像内使用 Node.js 22 和仓库提交的 `pnpm-lock.yaml` 安装依赖，因此不会使用宿主机 Node.js 运行时，也不会受宿主机 CPU 架构影响。NukemAI 不需要旧的 `sqlite`/`sqlite3` 包，它们不属于源码安装路径。
...
如果 `nukemai-home` volume 中还没有 `/root/.nukemai/nukemai.yaml`，entrypoint 会在首次启动时根据 `NUKEMAI_*` 环境变量生成配置。
...
运行 `docker compose up` 前，也可以设置 `NUKEMAI_WORKSPACE=/path/to/project`。
...
| 变量 | 说明 | 默认值 |
|---|---|---|
| `PILOT_HOME` | 容器内 NukemAI 状态目录 | `/root/.nukemai` |
| `NUKEMAI_MODEL` | 主模型标识，格式为 `provider/model` | `openrouter/deepseek/deepseek-v4-flash` |
| `NUKEMAI_LIGHT_MODEL` | 路由/判别用轻量模型标识 | `openrouter/qwen/qwen3-8b` |
| `NUKEMAI_API_KEY` | 主模型 Provider API Key | `PLACEHOLDER_RUN_ONBOARDING_TO_REPLACE` |
| `NUKEMAI_API_URL` | 主模型 Provider API Base URL | `https://openrouter.ai/api/v1` |
| `NUKEMAI_LIGHT_API_KEY` | 轻量模型使用不同 Provider 时的 API Key | 回退到 `NUKEMAI_API_KEY` |
| `NUKEMAI_LIGHT_API_URL` | 轻量模型使用不同 Provider 时的 API Base URL | 回退到 `NUKEMAI_API_URL` |
| `NUKEMAI_PROXY` | HTTP/HTTPS 代理 URL | — |
| `SERVER_PORT` | UI server 端口 | `3001` |
| `NUKEMAI_GATEWAY_PORT` | UI bridge 使用的 Gateway 端口 | `18789` |
