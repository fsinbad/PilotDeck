...
const RESERVED_NUKEMAI_PATTERN =
  /(?:official[^a-z0-9]*(nukemai|pilot)|(?:nukemai|pilot)[^a-z0-9]*official|^(?:nukemai|pilot)[^a-z0-9]*(marketplace|plugins|official))/iu;
...
export function validateMarketplaceName(name: string): string | undefined {
  ...
  if (NON_ASCII_PATTERN.test(name) || RESERVED_NUKEMAI_PATTERN.test(name)) {
    return "Marketplace name impersonates an official NukemAI marketplace.";
  }
  return undefined;
}
