#!/usr/bin/env node
/**
 * Bootstrap ~/.nukemai/nukemai.yaml when it doesn't exist yet, so the
 ...
 *          that hasUsableNukemAIConfig() recognises as "not ready" so the
 ...
 * Skip the whole step via $NUKEMAI_SKIP_BOOTSTRAP=1.
 */
...
const BOOTSTRAP_YAML = `# Auto-generated by scripts/bootstrap-nukemai-config.mjs.
...
function syncRepoSkillsToPilotHome(pilotHome) {
  ...
  for (const sourceDir of skillDirs) {
    ...
    if (previous) {
      ...
      console.warn(
        `[nukemai] Skipping repo skill '${slug}' at ${sourceDir}; slug already claimed by ${previous}.`,
      );
      continue;
    }
    ...
    try {
      ...
      console.warn(
        `[nukemai] Could not import repo skill '${slug}' into ${targetPath}: ${
  ...
  for (const { key, snippet } of PATCH_SECTIONS) {
    ...
    if (!pattern.test(content)) {
      try {
        ...
        patched = true;
        console.log(`[nukemai] Appended missing "${key}" section to ${configPath}.`);
      } catch (error) {
        console.warn(
          `[nukemai] Could not append "${key}" to ${configPath}: ${
            error instanceof Error ? error.message : String(error)
          }`,
        );
      }
...
function main() {
  if (process.env.NUKEMAI_SKIP_BOOTSTRAP === '1') {
    return;
  }
  ...
  if (skillSync.created > 0 || skillSync.skippedExisting > 0 || skillSync.skippedDuplicateSlug > 0) {
    console.log(
      `[nukemai] Synced repo skills into ${join(pilotHome, 'skills')}: ` +
  ...
  try {
    ...
    console.log(
      `[nukemai] No config at ${configPath}; wrote a placeholder so the gateway can boot.`,
    );
    console.log('[nukemai] Open the Web UI to finish onboarding (provider + API key).');
  } catch (error) {
    console.warn(
      `[nukemai] Could not bootstrap ${configPath}: ${error instanceof Error ? error.message : String(error)}`,
    );
    console.warn('[nukemai] You may need to create it manually before running npm run dev.');
  }
}
